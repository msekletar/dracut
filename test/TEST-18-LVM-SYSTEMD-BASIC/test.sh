#!/bin/bash
TEST_DESCRIPTION="root filesystem on LVM PV with new LVM module using systemd and event-based activation"
...
test_setup() {
    ...
    dd if=/dev/zero of=$TESTDIR/root.ext3 bs=1M count=92
    ...
    (
        ...
        . $basedir/dracut-init.sh
        inst_multiple sfdisk mkfs.ext3 poweroff cp umount grep dmsetup dd
    ...
    $basedir/dracut.sh -l -i $TESTDIR/overlay / \
                       -m "dash udev-rules base rootfs-block fs-lib kernel-modules dracut-systemd systemd-initrd systemd qemu" \
    ...
    $testdir/run-qemu -drive format=raw,index=0,media=disk,file=$TESTDIR/root.ext3 \
